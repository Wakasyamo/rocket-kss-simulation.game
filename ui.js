...
class UI {
  ...
  static renderPartPanel(partType, rocket, stage, onUpdate) {
    ...
    switch (partType) {
      case "nose": {
        ...
        add(UI.createMaterialList({ partType: "nose", part: rocket.nose, onSelect: v => { rocket.nose.material = v; onUpdate(); } }));
      ...
      case "body": {
        ...
        add(UI.createMaterialList({ partType: "body", part: rocket.body, onSelect: v => { rocket.body.material = v; onUpdate(); } }));
      ...
      case "transition": {
        ...
        add(UI.createChipRow({
          ...
          onSelect: v => { rocket.setTransitionEnabled(v === "on"); onUpdate(); }
        }));
        if (rocket.transitionEnabled) {
          ...
          add(UI.createMaterialList({ partType: "transition", part: rocket.transition, onSelect: v => { rocket.transition.material = v; onUpdate(); } }));
      ...
      case "fin": {
        ...
        add(UI.createMaterialList({ partType: "fin", part: rocket.fins, onSelect: v => { rocket.fins.material = v; onUpdate(); } }));
  ...
  /* ============================================================
     設計画面: データグリッド（初心者ステージではCNα等を非表示）
  ============================================================ */
  static updateDataGrid(rocket, stage) {
    ...
    if (stage.simplifiedUI) {
      // 初心者ステージ: 専門パラメータ(CG/CP/Cnα)は隠し、直感的な安定性ラベルのみ表示
      ...
      cells.push({ label: "Cnα", value: rocket.cnAlpha.toFixed(2) });
    }
