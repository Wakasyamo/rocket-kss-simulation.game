...
============================================================ */
class NoseCone {
  constructor({ length = 0.12, diameter = 0.024, material = "バルサ材", shape = "オジーブ", color = DEFAULT_COLORS.nose } = {}) {
    this.length = length;
  ...
  get mass() { return this.massForMaterial(this.material) / 1000; }
...
============================================================ */
class BodyTube {
  constructor({ length = 0.30, diameter = 0.024, material = "紙", color = DEFAULT_COLORS.body } = {}) {
    this.length = length;
    this.diameter = diameter;
    this.material = material;
    this.color = color;
...
============================================================ */
class Transition {
  constructor({ length = 0.05, diameterFront = 0.024, diameterBack = 0.018, material = "バルサ材", color = DEFAULT_COLORS.transition } = {}) {
    this.length = length;
...
============================================================ */
class Fin {
  constructor({ count = 3, shape = "標準", section = "矩形", material = "バルサ材", color = DEFAULT_COLORS.fin } = {}) {
    this.count = count;
    this.shape = shape;
...
============================================================ */
class Parachute {
  constructor({ diameter = 0.30 } = {}) { this.diameter = diameter; }
  get area() { const r = this.diameter / 2; return Math.PI * r * r; }
  get mass() { return (this.diameter * 100 * MASS_COEF.parachutePerCm) / 1000; }
...
class Engine {
  ...
  get mass() { return (ENGINE_MASS_MAP[this.classKey] || 30) / 1000; }
...
class Rocket {
  ...
  toJSON() {
    return {
      nose: { length: this.nose.length, diameter: this.nose.diameter, material: this.nose.material, shape: this.nose.shape, color: this.nose.color },
      body: { length: this.body.length, diameter: this.body.diameter, material: this.body.material, color: this.body.color },
      fins: {
        count: this.fins.count, shape: this.fins.shape, section: this.fins.section, material: this.fins.material,
        color: this.fins.color, customGeometry: this.fins.customGeometry
      },
      parachute: { diameter: this.parachute.diameter },
      ...
      transition: { length: this.transition.length, diameterFront: this.transition.diameterFront, diameterBack: this.transition.diameterBack, material: this.transition.material, color: this.transition.color },
