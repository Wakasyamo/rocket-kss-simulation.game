...
class Physics {
  ...
  static step(simState, rocket, stage, dt) {
    ...
    // ---- 記録更新 ----
    if (simState.y > simState.maxAltitude) simState.maxAltitude = simState.y;
...
/* ============================================================
   Physics.FlightRecorder
   ------------------------------------------------------------
   0.3の要件どおり Float32Array のリングバッファでサンプリングする。
   物理刻みは1/60s(=最大60Hz)だが、記録は指定hzに間引いて保存量を抑える。
============================================================ */
Physics.FlightRecorder = class FlightRecorder {
  /**
   * @param capacitySeconds 想定される最大飛行時間[s]（これを超えると古いものから上書き）
   * @param hz 記録サンプリングレート[Hz]
   */
  constructor(capacitySeconds = 180, hz = 30) {
    this.hz = hz;
    this.capacity = Math.ceil(capacitySeconds * hz);
    this.t = new Float32Array(this.capacity);
    ...
    this._writeIndex = 0;   // 次に書き込む位置（リング）
    this._count = 0;        // 実際に書き込まれた件数（capacity上限あり）
    this._lastRecordedT = -Infinity;
  }

  /** simStateの現在値を、hzで指定した間隔に間引いて記録する */
  record(simState) {
    const interval = 1 / this.hz;
    if (simState.t - this._lastRecordedT < interval) return;
    this._lastRecordedT = simState.t;

    const i = this._writeIndex % this.capacity;
    this.t[i] = simState.t;
    this.x[i] = simState.x;
    this.y[i] = simState.y;
    this.vx[i] = simState.vx;
    this.vy[i] = simState.vy;
    this.ax[i] = simState.ax;
    this.ay[i] = simState.ay;
  ...
  /**
   * CSV出力等のために、書き込み順に並んだ通常配列へ変換して返す。
   * リングバッファが一周している場合も正しい時系列順に並べ替える。
   */
  toArrays() {
    const n = this._count;
    const startIndex = this._count < this.capacity ? 0 : this._writeIndex % this.capacity;
    const out = { t: new Array(n), x: new Array(n), y: new Array(n), vx: new Array(n), vy: new Array(n), ax: new Array(n), ay: new Array(n) };
    for (let k = 0; k < n; k++) {
      const i = (startIndex + k) % this.capacity;
      out.t[k] = this.t[i]; out.x[k] = this.x[i]; out.y[k] = this.y[i];
      out.vx[k] = this.vx[i]; out.vy[k] = this.vy[i];
      out.ax[k] = this.ax[i]; out.ay[k] = this.ay[i];
    }
    return out;
  }
};
