...
class Stage {
  constructor({
    ...
    allowMassOverride = false,
    tutorialImages = null,
    customPhysicsUnlocked = false
  }) {
    this.id = id;
    this.name = name;
    ...
    this.allowMassOverride = allowMassOverride;    // ステージ6限定: 各パーツの重量を手動上書きできるか

    // チュートリアル解説ダイアログに埋め込む画像URL（機能追加要件）。
    // 例: { design: "assets/tutorial-design.png", launch: null }
    // phase('design'|'launch')ごとに個別指定でき、未設定(null)ならimgは表示しない。
    this.tutorialImages = tutorialImages;

    // ステージ6限定: 発射待機画面で風速範囲・重力倍率をユーザーが
    // 自由に調整できるUIを表示するか
    this.customPhysicsUnlocked = customPhysicsUnlocked;
...
const STAGES = [
  ...
  new Stage({
    ...
    // 設計フェーズのチュートリアルに埋め込む解説画像（未配置でも自動非表示になる）
    tutorialImages: { design: "assets/tutorial-design-stage1.png", launch: "assets/tutorial-launch-stage1.png" }
  }),
  ...
  new Stage({
    ...
    allowMassOverride: true, // 各部品の重量を自由に上書き可能
    customPhysicsUnlocked: true // 風速・重力を打ち上げ画面で自由に調整可能
  })
];
